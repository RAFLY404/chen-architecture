...
export default function AboutUs() {
  ...
  useEffect(() => {
    fetch(getApiUrl('/team'))
      ...
      .then((data) => {
        setTeamMembers(data && data.length > 0 ? data : fallbackTeamMembers);
      })
      .catch((err) => {
  ...
  const aboutParagraphs = settings.aboutParagraphs?.length ? settings.aboutParagraphs : [
    'ACEN is a Jakarta-based group of architects, designers, and thinkers operating within architecture, urbanism, research, and development.',
    "Formed from the curiosity of advancing Tropical Architecture, ACEN believes architecture can shape better responses to today's challenges.",
    'ACEN explores and experiments to redefine the value of architecture in both globalization and localization.',
  ];
      ...
      <div className="max-w-[1400px] mx-auto px-6 sm:px-10 lg:px-16 pt-28 sm:pt-36 pb-0 pointer-events-auto">
        <motion.section {...fadeUp} className="mb-20 md:mb-28">
          ...
          <div className="w-full aspect-[16/9] md:aspect-[1580/1117] rounded-lg overflow-hidden mb-12 md:mb-16 relative bg-stone-100 dark:bg-[#151210]">
            ...
            />
            <img
              src={resolveImageUrl(settings.aboutHeroImage)}
              alt="ACEN Studio"
        ...
        <motion.section {...fadeUp} className="mb-16 md:mb-24">
          <h4 className="font-karla text-sm tracking-[0.15em] text-center text-black dark:text-[#e6e0d8] mb-2">
            {settings.teamSectionTitle || 'The Team'}
          </h4>
              ...
              >
                <div className="relative w-full aspect-[3/4] overflow-hidden rounded-sm bg-stone-100 dark:bg-[#151210] mb-4">
                  ...
                  {member.instagram && (
                    ...
                    >
                      <span className="font-karla text-[10px] text-stone-800 dark:text-stone-200">IG</span>
                    </a>
                  )}
                </div>
        ...
        <motion.section {...fadeUp} className="mb-20 md:mb-28">
          <h4 className="font-karla text-sm tracking-[0.15em] text-center text-black dark:text-[#e6e0d8] mb-8 md:mb-10">
            {settings.publicationsSectionTitle || 'Featured In'}
          </h4>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-5 sm:gap-6 items-center justify-items-center px-4 sm:px-8">
            {(settings.publications || []).map((name) => (
              <div
                key={name}
